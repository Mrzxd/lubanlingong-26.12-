...
@property (nonatomic, strong) NSDictionary *nextDiction;


@end

@implementation VerificationCardController {
    NSDictionary *typeCardDiction;
    NSDictionary *contentDiction;
}
...
- (void)viewDidLoad {
    ...
    //cardNo=6217002430035835629&cardBinCheck=true
}
- (void)netWorking {
   typeCardDiction = @{
        @"DC":@"储蓄卡",
   ...
   contentDiction =  @{
    ...
    WeakSelf;
    [ZXD_NetWorking getWithUrl:[NSString stringWithFormat:@"https://ccdcapi.alipay.com/validateAndCacheCardInfo.json"] params:@{
        @"_input_charset":@"utf-8",
        @"cardNo":_infoArray[1],
        @"cardBinCheck":@"true"
    } success:^(id  _Nonnull respones) {
        if (respones && respones[@"validated"] && [respones[@"validated"] intValue] == 1) {
            weakSelf.diction = respones;
            [weakSelf addSubView];
        } else {
            [WHToast showErrorWithMessage:@"卡号不正确"];
            [weakSelf.navigationController popViewControllerAnimated:YES];
        }
    } fail:^(NSError * _Nonnull error) {
        [WHToast showErrorWithMessage:@"网络错误，验证失败"];
    } showHUD:YES];
...
- (void)addSubView {
    ...
    _cardType = typeCardDiction[_diction[@"cardType"]];
    ...
    cashDetailLabel.text = contentDiction[_diction[@"bank"]] NonNull;
    ...
    amountDetailLabel.text = _diction[@"key"];
...
- (void)cardButtonAction:(UIButton *)button {
    ...
    _nextDiction = @{
        ...
        @"bankCard":_diction[@"key"],
        @"cardId":_infoArray[2],
        @"bank":[contentDiction[_diction[@"bank"]] stringByAppendingString:_cardType],
        @"bankPhone":_textField.text,
    };
      SMSVerificationCodeController *svc = [SMSVerificationCodeController new];
      svc.cardDictionary = _nextDiction;
