...
-(void)instance{
    ...
    //segmentTapView
    self.segmentTapView = [[SegmentTapView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.view.frame), 30*ScalePpth)];
    self.segmentTapView.delegate = self;
    [self.view addSubview:self.segmentTapView];
    
    //pageController
    self.pageController = [[UIPageViewController alloc] initWithTransitionStyle:UIPageViewControllerTransitionStyleScroll navigationOrientation:UIPageViewControllerNavigationOrientationHorizontal options:nil];
    self.pageController.view.frame = CGRectMake(0, 30*ScalePpth, CGRectGetWidth(self.view.frame), CGRectGetHeight(self.view.frame) - 30*ScalePpth);
...
- (void)willSelectedIndex:(int)index {    
    [self.pageController setViewControllers:@[self.viewControllers[index]] direction:UIPageViewControllerNavigationDirectionReverse animated:NO completion:nil];
    [self.segmentTapView setSelectedIndex:index];
}
